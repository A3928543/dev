--######################################################################################################################################## 
--# 
--# 	Procedimiento para reversar un respaldo de swaps con todas sus dependencias físicas y lógicas 
--# 
--########################################################################################################################################
...
DECLARE
	...
	-- Obtiene todos los identificadores de respaldos generados para tipos de respaldo de SWAPS
	 CURSOR cur_all_resplds IS
		SELECT id_respaldo
		FROM sica_admin.sc_respaldo
		WHERE tipo_respaldo = C_TIPO_RESPALDO_SWAP AND FECHA_RESTAURACION IS NULL;
	...
	--	
	--	params:
	--		p_id_tabla NUMBER: Nombre de la tabla cuyas columnas se desea encontrar
    ...
    -----------
	--	Procedimiento para hacer la restauración del respaldo de la tabla
	--
	--	params:
	--		p_id_respaldo: Identificador del respaldo
	--		table_to_backup: Registro que identifica la tabla cuyos registros se van a restaurar
	--		tabla_reversada: Indica si se llevó a cabo la restauración o no (dependiendo si existen resgistros que restaurar o no)
	-----------
	PROCEDURE rev_respld_tabla (
		p_id_respaldo IN NUMBER := 0,
	...
	--		p_id_respaldo: Identificador del respaldo
	------------
	PROCEDURE reversa_todo(p_id_respaldo NUMBER) IS
	...
	BEGIN
		DBMS_OUTPUT.PUT_LINE('Comienza ejecución de restauración de respaldo de Swaps');
		log_respld(p_id_respaldo, NULL, C_TIPO_MSG_INFO, 'Comienza restauración de respaldo de Swaps, id_respaldo=' || p_id_respaldo);
		...
		log_respld(p_id_respaldo, NULL, C_TIPO_MSG_INFO, 'Finaliza restauración de Swaps');
		DBMS_OUTPUT.PUT_LINE('Finaliza ejecución de restauración de respaldo de Swaps');
	EXCEPTION
		WHEN OTHERS THEN
...
BEGIN
	...
	-- Recuperación selectiva de respaldos de swaps, llamar al procedimiento de recuperación por cada respaldo
	reversa_todo(1);
	reversa_todo(2);
	reversa_todo(3);
	-- reversa_todo(n);
