--######################################################################################################################################## 
--# 
--# 	Procedimiento para respaldar swaps cancelados con todas sus dependencias físicas y lógicas 
--# 
--########################################################################################################################################
...
DECLARE
	...
	BEGIN
		...
		-- Contruye la senetencia SQL de selección para extraer los datos a enviar a histórico
		-- SWAP cancelado
		...
		log_respld(p_id_respaldo, table_to_backup.id_tabla, C_TIPO_MSG_INFO, 'Ejecutando inserts en tabla histórica actual');
	...
	BEGIN
		...
		log_respld(p_id_respaldo, rec_tbls_to_delete.id_tabla, C_TIPO_MSG_INFO, 'Eliminación completa');
		...
		log_respld(p_id_respaldo, rec_tbls_to_delete.id_tabla, C_TIPO_MSG_INFO, 'Registros después de eliminar: ' || rows_after_delete);
	...
	-------------
	 ...
	 BEGIN
		DBMS_OUTPUT.PUT_LINE('Comienza ejecución de respaldo de Swaps');
		SELECT *
		...
		END LOOP;
		log_respld(g_id_respaldo, NULL, C_TIPO_MSG_INFO, 'Finaliza proceso de inserción en tablas históricas, tablas procesadas: ' || tablas_procesadas);
		...
		DBMS_OUTPUT.PUT_LINE('Termina ejecución de respaldo de Swaps');
	 EXCEPTION
		WHEN OTHERS THEN
...
BEGIN
	-- Llamadas para producción, descomentar
	-- 2007
	...
	-- Llamada al procedimiento para prueba en ambiente local: Comentar línea para ejecución en producción
